...
def buy_message(message):
    ...
    if count:
        bot.send_message(message.from_user.id, "📂 Доступные категории Cлито в @END_Software", reply_markup=keyboard)
    else:
...
def send_support(message, support_theme):
    if not_const(message.text):
        if message.text is not None:
            ...
            msg = "❗ <b>Обращение в поддержку</b>\n" \
                  f"🔹 ID Пользователя: <b>{message.from_user.username}</b>\n" \
